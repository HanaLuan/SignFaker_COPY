...
fun Routing.energy() {
    get("/custom_energy") {
        val data = call.request.queryParameters["data"] ?: throw BadRequestException("Missing 'data'")
        val salt = call.request.queryParameters["salt"]?.hex2ByteArray() ?: throw BadRequestException("Missing 'salt'")
    ...
    get("/energy") {
        ...
        if (!(data.startsWith("810_") || data.startsWith("812_"))) {
            throw BadRequestException("data参数不合法")
        }

        val mode = call.request.queryParameters["mode"] ?: when (data) {
            "810_d", "810_a", "810_f", "810_9" -> "v2"
            "810_2", "810_25", "810_7", "810_24" -> "v1"
            "812_b", "812_a" -> "v3"
            "812_5" -> "v4"
            else -> throw BadRequestException("无法自动决断mode，请主动提供")
        }
        ...
        val salt = when (mode) {
            "v1" -> {
                val uin = call.request.queryParameters["uin"]?.toLong() ?: throw BadRequestException("Missing 'uin'")
                val version = call.request.queryParameters["version"] ?: throw BadRequestException("Missing 'version'")
                val guid = call.request.queryParameters["guid"]?.hex2ByteArray() ?: throw BadRequestException("Missing 'guid'")
            ...
            "v3" -> { // 812_a
                val version = call.request.queryParameters["version"] ?: throw BadRequestException("Missing 'version'")
                val phone = call.request.queryParameters["phone"]?.toByteArray() ?: throw BadRequestException("Missing 'phone'")
            ...
            "v4" -> { // 812_5
                val receipt = call.request.queryParameters["receipt"]?.toByteArray() ?: throw BadRequestException("Missing 'receipt'")
                val code = call.request.queryParameters["code"] ?: throw BadRequestException("Missing 'code'")
